...
# Fallback metrics when scikit-learn is not installed in the environment (e.g. Python 3.14 on Windows)
try:
...
# Support either baseline_llm_predictions_v2.jsonl or baseline_llm_predictions.jsonl
INPUT_V2 = Path("data/golden/baseline_llm_predictions_v2.jsonl")
INPUT_DEFAULT = Path("data/golden/baseline_llm_predictions.jsonl")

INPUT = INPUT_V2 if INPUT_V2.exists() else INPUT_DEFAULT


def main():
    if not INPUT.exists():
        raise FileNotFoundError(
            f"Prediction file not found. Tried:\n  - {INPUT_V2}\n  - {INPUT_DEFAULT}"
        )
    ...
    print("=" * 80)
    print("BASELINE 2 — LLM ZERO-SHOT + FULL CONVERSATION CONTEXT")
    print(f"File evaluated: {INPUT}")
    print("=" * 80)
